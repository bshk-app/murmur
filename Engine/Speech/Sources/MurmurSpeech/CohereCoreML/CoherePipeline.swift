import MurmurCore
// Adapted from FluidInference/FluidAudio, revision 5c19d5e12320e22bbfb7a1877b089d2665a69add.
...
private let pipelineLogger = Logger(subsystem: "app.bshk.murmur.prototype", category: "CoherePipeline")
    ...
    public func compute(audio: [Float]) -> Output {
        ...
        for frame in 0..<nFrames {
            ...
            padded.withUnsafeBufferPointer { input in
                vDSP_vmul(input.baseAddress!.advanced(by: start), 1,
                          paddedWindow, 1, &windowed, 1, vDSP_Length(nFFT))
            }
...
public actor CoherePipeline {
    ...
    public enum DecoderVariant: Sendable {
        ...
        public var compiledFileName: String {
            switch self {
            case .v2: return "cohere_decoder_cache_external_v2.mlmodelc"
            case .v1: return "cohere_decoder_cache_external.mlmodelc"
            }
        }
    ...
    public static func loadModels(
        ...
        onPreparation: @escaping @MainActor @Sendable (String) -> Void = { _ in }
    ) async throws -> LoadedModels {
        let config = MLModelConfiguration()
        config.computeUnits = computeUnits

        let manifestURL = encoderDir.appendingPathComponent("encoder_manifest.json")
        let encoderURLs: [URL]
        if FileManager.default.fileExists(atPath: manifestURL.path) {
            struct Manifest: Decodable { let stages: [String] }
            let manifest = try JSONDecoder().decode(Manifest.self, from: Data(contentsOf: manifestURL))
            guard !manifest.stages.isEmpty, manifest.stages.count <= 16,
                  manifest.stages.allSatisfy({ $0 == URL(fileURLWithPath: $0).lastPathComponent && $0.hasSuffix(".mlmodelc") }) else {
                throw CohereAsrError.modelNotFound("Invalid encoder stage manifest")
            }
            encoderURLs = manifest.stages.map { encoderDir.appendingPathComponent($0) }
        } else {
            encoderURLs = [encoderDir.appendingPathComponent("cohere_encoder.mlmodelc")]
        }
        ...
        // Specialize one bounded stage at a time. Do not retain every stage's
        // CPU/ANE working weights: monolithic loading exceeds the iPhone limit.
        for (index, url) in encoderURLs.enumerated() {
            await onPreparation("Core ML: энкодер \(index + 1)/\(encoderURLs.count)")
            print("COREML_ENCODER_PREPARE \(index + 1)/\(encoderURLs.count) \(url.lastPathComponent)")
            fflush(stdout)
            try autoreleasepool { _ = try MLModel(contentsOf: url, configuration: config) }
        }
        await onPreparation("Core ML: декодер")
    ...
    public func transcribe(
        ...
        let (encoderHidden, encoderValid) = try await runEncoder(
            paddedMel: paddedMel, featureLength: featureLength, encoderURLs: models.encoderURLs, computeUnits: models.computeUnits)
    ...
    private func runEncoder(
        paddedMel: [[Float]], featureLength: Int, encoderURLs: [URL], computeUnits: MLComputeUnits
        ...
        for (index, url) in encoderURLs.enumerated() {
            ...
            hidden = try autoreleasepool {
                ...
                let provider = try MLDictionaryFeatureProvider(dictionary: [
                    index == 0 ? "input_features" : "hidden_states_in": MLFeatureValue(multiArray: input),
                    "feature_length": MLFeatureValue(multiArray: featLen),
                ])
                let out = try Self.predictSynchronously(encoder, from: provider)
                guard let output = out.featureValue(for: "hidden_states")?.multiArrayValue else {
                    throw CohereAsrError.encodingFailed("Encoder output missing 'hidden_states'")
                }
                // Detach the small activation from the prediction/weight owner
                // before releasing this model and loading the following stage.
                return try Self.copyActivation(output)
            }
        }
    ...
    /// Three stride-2 convolutions: ceil(feature_length / 8).
    /// Scaling by 438/3500 incorrectly admits a padded frame for exact multiples.
    public static func encoderValidFrames(
        featureLength: Int, encoderSeqLen: Int
    ) -> Int {
        let raw = (featureLength + 7) / 8
    ...
    private func decodeCacheExternal(
        ...
        for step in 0..<effectiveMax {
            ...
            let out = try Self.predictSynchronously(decoder, from: provider)
    ...
    private static func copyActivation(_ input: MLMultiArray) throws -> MLMultiArray {
        let shape = input.shape.map(\.intValue), strides = input.strides.map(\.intValue)
        guard shape.count == 3, shape[0] == 1,
              input.dataType == .float16 || input.dataType == .float32 else {
            throw CohereAsrError.encodingFailed("Unexpected encoder activation shape or dtype")
        }
        let result = try MLMultiArray(shape: input.shape, dataType: input.dataType)
        let width = input.dataType == .float16 ? 2 : 4
        for row in 0..<shape[1] {
            if strides[2] == 1 {
                result.dataPointer.advanced(by: row * shape[2] * width).copyMemory(
                    from: input.dataPointer.advanced(by: row * strides[1] * width), byteCount: shape[2] * width)
            } else {
                for column in 0..<shape[2] {
                    result.dataPointer.advanced(by: (row * shape[2] + column) * width).copyMemory(
                        from: input.dataPointer.advanced(by: (row * strides[1] + column * strides[2]) * width), byteCount: width)
                }
            }
        }
        return result
    ...
    private static func predictSynchronously(_ model: MLModel, from provider: MLFeatureProvider) throws -> MLFeatureProvider {

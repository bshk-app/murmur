...
public struct CohereTranscribeConfig: Decodable, Sendable {
    ...
    public init(from decoder: Swift.Decoder) throws {
        ...
        if let explicit = try container.decodeIfPresent(Int.self, forKey: .vocabSize) {
            vocabSize = explicit
        } else {
            struct Head: Decodable { let num_classes: Int }
            vocabSize = try container.decode(Head.self, forKey: .head).num_classes
        }

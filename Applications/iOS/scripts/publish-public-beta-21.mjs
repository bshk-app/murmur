...
const groupID='d79330c5-ed46-4449-aafe-5963bee69041',appID='6809815142',kid='38V7FDZ48J';
const issuer=execFileSync('op',['read','op://Private/o5ocl3hneq2mobu6exk4vtbqhu/Issuer ID'],{encoding:'utf8',timeout:60000,stdio:['ignore','pipe','inherit']}).trim();
...
const candidates=await api('/v1/builds?filter[app]='+appID+'&filter[version]=21&include=preReleaseVersion');
const matches=candidates.data.filter(x=>candidates.included?.some(v=>v.type==='preReleaseVersions'&&v.id===x.relationships.preReleaseVersion.data.id&&v.attributes.version==='1.0.0'));
if(matches.length!==1)throw Error('Expected exactly one 1.0.0 (21) build');
const buildID=matches[0].id;
...
if(b.data.attributes.version!=='21'||b.data.relationships.app.data.id!==appID||b.data.attributes.processingState!=='VALID')throw Error('Unexpected build or processing state');
...
const en=loc.data.find(x=>x.attributes.locale==='en-US');const description=fs.readFileSync('Applications/iOS/Release/1.0.0-21/TestFlight.en.md','utf8');
if(!en)await api('/v1/betaBuildLocalizations','POST',{data:{type:'betaBuildLocalizations',attributes:{locale:'en-US',whatsNew:description},relationships:{build:{data:{type:'builds',id:buildID}}}}});
else if(en.attributes.whatsNew!==description)await api('/v1/betaBuildLocalizations/'+en.id,'PATCH',{data:{type:'betaBuildLocalizations',id:en.id,attributes:{whatsNew:description}}});
const membership=await api('/v1/betaGroups/'+groupID+'/relationships/builds');
if(!membership.data.some(x=>x.id===buildID)){await api('/v1/builds/'+buildID+'/relationships/betaGroups','POST',{data:[{type:'betaGroups',id:groupID}]});console.log('Build 21 added to Murmator Beta.');}

...
const archive = path.resolve(process.argv[2] ?? path.join(root, 'build/Archives/Murmator-1.0.0-22.xcarchive'));
...
assert.equal(info.CFBundleVersion,'22');
...
if(exported) {
    ...
    for (const content of [distribution,...distribution.embeddedBinaries]) {
        ...
        assert.equal(content.buildNumber,'22');
    ...
    assert.equal(plist(path.join(payload,'Info.plist')).CFBundleVersion,'22');
...
const snapshotPath=path.join(root,'Release/1.0.0-22/archive-inputs.json');
...
const validationPath=path.join(root,'Release/1.0.0-22/validation.json');
...
const uploadPath=path.join(root,'Release/1.0.0-22/upload.json');

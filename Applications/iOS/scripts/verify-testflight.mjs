...
const archive = path.resolve(process.argv[2] ?? path.join(root, 'build/Archives/Murmator-1.0.0-11.xcarchive'));
...
assert.equal(info.CFBundleVersion,'11');
...
assert.deepEqual(plugins,['MurMurKeyboard.appex','MurMurWidgets.appex']);
...
if(exported) {
    ...
    for (const content of [distribution,...distribution.embeddedBinaries]) {
        ...
        assert.equal(content.buildNumber,'11');
        assert.equal(content.versionNumber,'1.0.0');
        const groups=content.entitlements['com.apple.security.application-groups'];
        const expected=content.name==='MurMurTranslationProvider.appex' ? ['group.app.bshk.murmur.ios.translation'] : content.name==='MurMurMobile.app' ? ['group.app.bshk.murmur.ios.shared','group.app.bshk.murmur.ios.translation'] : ['group.app.bshk.murmur.ios.shared'];
    ...
    assert.equal(plist(path.join(payload,'Info.plist')).CFBundleVersion,'11');
...
const snapshotPath=path.join(root,'Release/archive-inputs.json');
...
fs.writeFileSync(path.join(root,'Release/validation.json'),JSON.stringify(report,null,2)+'\n');

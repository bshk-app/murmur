...
const archive = path.resolve(process.argv[2] ?? path.join(root, 'build/Archives/Murmator-1.0.0-18.xcarchive'));
...
assert.equal(info.CFBundleVersion,'18');
...
if(exported) {
    ...
    for (const content of [distribution,...distribution.embeddedBinaries]) {
        ...
        assert.equal(content.buildNumber,'18');
    ...
    assert.equal(plist(path.join(payload,'Info.plist')).CFBundleVersion,'18');
...
const snapshotPath=path.join(root,'Release/1.0.0-18/archive-inputs.json');
...
const validationPath=path.join(root,'Release/1.0.0-18/validation.json');
const prior=JSON.parse(fs.readFileSync(validationPath,'utf8'));
delete prior.exportBlocker;
Object.assign(report,{coreTests:prior.coreTests,domTests:prior.domTests,safariTests:prior.safariTests,safariTestsPassed:prior.safariTestsPassed,compactSheetTests:prior.compactSheetTests,compactSheetTestsPassed:prior.compactSheetTestsPassed,physicalDeviceTested:false,appStoreExported:!!exported});
const uploadPath=path.join(root,'Release/1.0.0-18/upload.json');

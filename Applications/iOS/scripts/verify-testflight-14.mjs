...
const archive = path.resolve(process.argv[2] ?? path.join(root, 'build/Archives/Murmator-1.0.0-14.xcarchive'));
...
assert.equal(info.CFBundleVersion,'14');
...
if(exported) {
    ...
    for (const content of [distribution,...distribution.embeddedBinaries]) {
        ...
        assert.equal(content.buildNumber,'14');
    ...
    assert.equal(plist(path.join(payload,'Info.plist')).CFBundleVersion,'14');
...
const snapshotPath=path.join(root,'Release/1.0.0-14/archive-inputs.json');
...
report.iconUIAppearancesVerified=["light","dark"];
const validationPath=path.join(root,'Release/1.0.0-14/validation.json');
const prior=JSON.parse(fs.readFileSync(validationPath,'utf8'));
delete prior.exportBlocker;
Object.assign(report,{coreTests:prior.coreTests,domTests:prior.domTests,safariTests:prior.safariTests,safariTestsPassed:prior.safariTestsPassed,coreAndPageTestBaselineBuild:"12",compactSheetTests:prior.compactSheetTests,compactSheetTestsPassed:prior.compactSheetTestsPassed,compactSheetTestBaselineBuild:"13",physicalDeviceTested:false,appStoreExported:!!exported});
const uploadPath=path.join(root,'Release/1.0.0-14/upload.json');

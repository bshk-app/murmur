...
const release = path.join(root, 'Release/1.0.0-17');
const locales = ['en', 'ru', 'de', 'es', 'fr', 'fi'];
const version = '1.0.0', build = '17', team = 'Q8H6GWJ658';
...
function checkSources() {
    ...
    assert.match(project, /let releaseBuild = "17"/);
...
if (exported) {
    ...
    const unpacked = fs.mkdtempSync(path.join(root, 'build/ipa-verification-17-'));
...
if (fs.existsSync(uploadPath)) {
    ...
    if (upload.version === version && upload.build === build && upload.uploaded === true) {
        ...
        report.appleProcessing = upload.appleProcessing ?? report.appleProcessing;
        report.testingGroupAvailability = upload.testFlightGroupAssignment ?? report.testingGroupAvailability;
        if (upload.internalBuildState) report.internalBuildState = upload.internalBuildState;
    }
}
fs.mkdirSync(release, {recursive: true});

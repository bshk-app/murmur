...
const archive = path.resolve(process.argv[2] ?? path.join(root, 'build/Archives/Murmator-1.0.0-13.xcarchive'));
...
assert.equal(info.CFBundleVersion,'13');
...
if(exported) {
    ...
    for (const content of [distribution,...distribution.embeddedBinaries]) {
        ...
        assert.equal(content.buildNumber,'13');
    ...
    assert.equal(plist(path.join(payload,'Info.plist')).CFBundleVersion,'13');
...
const snapshotPath=path.join(root,'Release/1.0.0-13/archive-inputs.json');

...
const release = path.join(root, 'Release/1.0.0-16');
const locales = ['en', 'ru', 'de', 'es', 'fr', 'fi'];
const version = '1.0.0', build = '16', team = 'Q8H6GWJ658';
...
function checkSources() {
    ...
    assert.match(project, /let releaseBuild = "16"/);
...
if (exported) {
    ...
    const unpacked = fs.mkdtempSync(path.join(root, 'build/ipa-verification-16-'));
